...
def sort_3d_array_indices_desc(arr):
    # Flatten the array and get the flattened indices that would sort the array
    flattened_indices = np.argsort(arr, axis=None)[::-1]
    # Convert the flattened indices to multi-dimensional indices
    multi_dim_indices = np.unravel_index(flattened_indices, arr.shape)
    # Combine the multi-dimensional indices into a list of tuples
    sorted_indices = list(zip(*multi_dim_indices))
